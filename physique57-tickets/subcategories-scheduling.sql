-- Subcategories with Dynamic Form Fields
-- Run this AFTER the main database-setup.sql

-- ============================================
-- SCHEDULING CATEGORY SUBCATEGORIES
-- ============================================

-- Get Scheduling category ID
DO $$
DECLARE
  scheduling_cat_id UUID;
  operations_team_id UUID;
BEGIN
  SELECT id INTO scheduling_cat_id FROM categories WHERE name = 'Scheduling';
  SELECT id INTO operations_team_id FROM teams WHERE name = 'Operations';
  ...
  RAISE NOTICE 'Scheduling subcategories with dynamic forms created successfully';
END $$;

-- ============================================
-- ADD STANDARD METADATA FIELDS TO ALL TICKETS
-- ============================================

-- These are captured at ticket creation, not in dynamic forms
-- Already in the main tickets table:
-- - studio_location (dropdown from Locations list)
-- - priority (dropdown: Low/Medium/High/Critical)
-- - customer_name, customer_email, customer_phone
-- - reported_by_user_id (auto-captured from logged in user)

-- Add client status and routing fields
ALTER TABLE tickets ADD COLUMN IF NOT EXISTS client_status VARCHAR(50);
ALTER TABLE tickets ADD COLUMN IF NOT EXISTS issue_category VARCHAR(100);
ALTER TABLE tickets ADD COLUMN IF NOT EXISTS department_routing VARCHAR(100);

-- Create lookup tables for standard options
...
INSERT INTO studio_locations (name) VALUES
...
-- Insert trainers
INSERT INTO trainer_list (name) VALUES
...
('Studio Cardio Barre'), ('Studio FIT'), ('Studio Mat 57'),
('Studio SWEAT In 30'), ('Studio Amped Up!'), ('Studio Back Body Blaze'),
('Studio Cardio Barre Plus'), ('Studio Cardio Barre Express'), ('Studio HIIT'),
('Studio Back Body Blaze Express'), ('Studio Recovery'), ('Studio Hosted Class'),
('Studio Trainer''s Choice'), ('Studio Pre/Post Natal'), ('Studio Mat 57 Express'),
('Studio PowerCycle Express'), ('Studio PowerCycle'), ('Studio Strength Lab (Pull)'),
('Studio Strength Lab (Full Body)'), ('Studio Strength Lab (Push)'), ('Studio Strength Lab');

RAISE NOTICE 'Dynamic forms and lookup tables created successfully!';
RAISE NOTICE 'Next: Add more subcategories for other categories following the same pattern';
